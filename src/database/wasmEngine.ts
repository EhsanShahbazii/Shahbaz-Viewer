...
export class SqliteWasmEngine implements ISqliteEngine {
  private static SQL: SqlJsStatic | null = null;
  private db: Database | null = null;
  private filePath: string = '';

  public static async init(wasmDir: string): Promise<void> {
    if (!SqliteWasmEngine.SQL) {
      SqliteWasmEngine.SQL = await initSqlJs({
  ...
  public load(filePath: string, fileBuffer?: Buffer | Uint8Array): void {
    if (!SqliteWasmEngine.SQL) {
      throw new Error('SQLite WebAssembly engine not initialized.');
    }
    if (!fileBuffer) {
      if (fs.existsSync(filePath)) {
        ...
        if (stats.size >= 2 * 1024 * 1024 * 1024) {
          throw new Error(
            `Database file size (${(stats.size / 1e9).toFixed(2)} GB) exceeds 2 GiB. WebAssembly memory cannot load databases this large.`
          );
        }
    ...
    this.db = new SqliteWasmEngine.SQL.Database(fileBuffer);
  }
  ...
  public explainQuery(query: string): ExplainResult {
    if (!this.db) {
      throw new Error('Database not loaded.');
    }
  ...
  public importData(options: ImportOptions): ImportResult {
    if (!this.db) {
      throw new Error('Database not loaded.');
    }
